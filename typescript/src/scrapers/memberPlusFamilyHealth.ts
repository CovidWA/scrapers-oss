...
const humanName = 'Member Plus Family Health';
const key = 'mpfh_ts';
const url = 'https://vaccinempfh.timetap.com/#/';

export class MemberPlusFamilyHealthScraper extends Scraper {
  constructor() {
    super(humanName, key);
  }
  ...
  private async checkVaccinationStatus(page: Page) {
    try {
      ...
      try {
        ...
        await page.waitForSelector(noTimesSelector, {timeout: 10000});
        console.log('Not seeing times to select');
        this.result = ScrapeResult.NO;
        return;
      } catch (err) {
        console.log(
